...
batch_size = 128  # bd5l fy kol iteration 128 pic
num_epochs = 170  # 3dd lfat al cnn 3shan y3ml train
learning_rate = 0.001  # speed train wl lw zad awi hy3ml loss fl accuracy
...
@app.route('/')
def index():
    return render_template('StartQuiz.php')


def construct_cnn():
    model = Sequential()  # way of creating deep learning models byd5ol 1 input wa by5rod output
    model.add(Conv2D(64, (3, 3), padding='same', input_shape=(48, 48, 1)))
    model.add(Activation('relu'))  # lazem n3mlha 3shan mn 8erha msh h3rf a3ml train
    model.add(MaxPooling2D(pool_size=(2, 2)))  # 3shan n2all al features elly mst5dmnhash
    ...
    model.add(Dropout(0.20))  # bt3ml shuffle ll dataset
    model.add(Flatten())  # bhy2 enha t5rog 1D
    model.add(Dense(256))  # hidden layer btgm3 kol al input neurals elly homa 256 mn al Con2d wa b3d kda bt3ml relu 3shan tmn3 moshkla e5tfa2 al mlam37
    model.add(Activation("relu"))
    model.add(BatchNormalization())
    model.add(Dropout(0.30))
    model.add(Dense(7, activation='softmax'))  # b3den btgm3 kol al layers fl softmax wa by7dd al FE
...
def startCapturing():
    ...
    graph = tf.get_default_graph()  # btgeb al rasm al eftrady ll tensorflow elly by7dd al face
    user_home = str(Path.home())
    ...
    while True:
        ...
        start_time = time.time()  # time al bd2 feh al python(seconds)
        # elasped_time = time.time() - start_time  # time al bd2 feh python run - nfs al time 3shan ytl3ly zero 3shan yt722 condition al while w
        while (time.time() - start_time) < capture_duration:
            rval, frame = webcam.read()  # will get frame from camera
            if rval == True:  # rval is a boolean variable that returns true if the frame is available.(htgeb al value mn frame al camera)
                ...
                for (x, y, w, h) in faces:
                    cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0),
                                  2)  # frame, startpoint, endpoint,color,thikness
                    face = gray[y:y + h, x:x + w]  # location oh face [start point : endpoint]
                    detected_face = cv2.resize(face, (48, 48),
                                               interpolation=cv2.INTER_AREA)  # interpolation a7san al anwa3 elly bt3ml resize
                    x = image.img_to_array(detected_face)
                    x = np.expand_dims(x, axis=0)  #
                    x /= 255  #######
                    with graph.as_default():  # bys5dm al rasm al eftrady ll was 3shan y predict al values l kol expression
                        ...
                        # enumerate hymshy bl trteb mn al expressions_category as list
                        for index, emotion in enumerate(expressions_category):
                            cv2.putText(frame, emotion, (10, index * 20 + 20), cv2.FONT_HERSHEY_PLAIN, 0.8, (0, 255, 0),
                                        lineType=cv2.LINE_AA)  # frame,esm emotion,startpoint(x=10, y   ,fonttype,thickness,color,linetype

                            cv2.rectangle(frame, (70, index * 20 + 10),
                                          (70 + int(detected_expression[index] * 100), (index + 1) * 20 + 4),
                                          (0, 255, 0), -1)  # frame, startpoint, endpoint,color,thikness
        ...
        print("\nMean Probability of Each Expression are: ")
        ...
        conn = pymysql.connect(host='localhost', user='root', password='', db='e-learning')
        a = conn.cursor()
        sql = 'SELECT  `score`, `test_time`, `level` FROM `mst_result`'
        ...
        merge = np.append(arr_of_web, arr_of_facial).reshape(-10, 10)

        pred = model.predict(merge)
        print("The Next Level is: ", pred)
        # name of csv file
        filename = "NextLevel.csv"
        file_exists = os.path.isfile(filename)
        fields = ['Next Level']
        ...
        with open(filename, 'a', newline='') as csvfile:
            # creating a csv dict writer object
            field_name_level = csv.DictWriter(csvfile, fieldnames=fields)
            ...
            # writing headers (field names)
            if not file_exists:
                field_name_level.writeheader()

            # writing the data rows
            row_writer.writerow(pred)

        os.remove("Facial Expressions.csv")
        os.remove("Final Expression.csv")
...
def begin():
    ...
    Start = (render_template('pageresult.php'), startCapturing())
    return Start


@app.route('/finish')
def finish():
    End = (render_template('pageresult.php'), finishx())
    return End
